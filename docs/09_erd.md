...
| 컬럼명 | 실제 PostgreSQL DDL 기준 소문자 snake_case 사용 | created_at, updated_at, analysis_job_id |
...
현재 운영 분석은 RGB 이미지와 열화상 이미지를 각각 단건으로 처리하며, 분석 작업은 `image_id`를 기준으로 생성한다.

Pair 생성·관리 및 Fusion 분석은 현재 운영 범위와 ERD에서 제외한다.
...
| 정규화 단계 | 적용 여부 | 설명 |
| --- | --- | --- |
| 1정규형, 1NF | 충족 | 하나의 컬럼에 여러 값을 저장하지 않고, 이미지·결함 후보·검토 이력을 별도 테이블로 분리한다. |
...
| ANALYSIS_JOBS | inspection_id | image_id → INSPECTION_IMAGES.inspection_id를 통해 점검 회차를 알 수 있다. |
| ANALYSIS_RESULTS | inspection_id, plant_id, zone_id, equipment_id, target_type, input_type | analysis_job_id → ANALYSIS_JOBS → INSPECTION_IMAGES를 통해 대상 정보를 알 수 있다. |
...
점검 회차와 업로드 이미지 메타데이터를 관리하는 도메인이다.
...
| INSPECTIONS 1:N INSPECTION_IMAGES | 하나의 점검 회차에는 여러 RGB 또는 열화상 이미지가 업로드될 수 있다. |
...
| 항목 | 설명 |
| --- | --- |
| INSPECTIONS의 의미 | 단순 이미지 업로드 묶음이 아니라 특정 구역에 대한 점검 회차 또는 점검 이벤트이다. |
| INSPECTION_IMAGES의 의미 | 이미지 파일 자체가 아니라 이미지 메타데이터와 객체 저장소 경로를 저장한다. |
| 이미지 단건 관리 | RGB 이미지와 열화상 이미지는 각각 고유한 image_id를 가진 독립적인 이미지로 저장한다. |
| 이미지 유형 | image_type으로 RGB 또는 THERMAL을 구분한다. |
...
| 정규화 기준 | INSPECTION_IMAGES에는 plant_id와 zone_id를 직접 저장하지 않는다. 구역과 발전소는 inspection_id를 통해 조회한다. |
| 분석 대상 | AI 분석 작업은 INSPECTION_IMAGES.id에 해당하는 이미지 한 건을 기준으로 생성한다. |
...
현재 운영 분석 대상은 `image_id`로 식별되는 RGB 또는 열화상 이미지 한 건이다.
...
| 관계 | 설명 |
| --- | --- |
| INSPECTION_IMAGES 1:N ANALYSIS_JOBS | 하나의 이미지에 대해 재요청이나 모델 버전 변경 등에 따라 여러 분석 작업이 생성될 수 있다. |
...
| Table | Column | Description |
| --- | --- | --- |
| ANALYSIS_JOBS | image_id | RGB 또는 열화상 단건 분석 대상인 INSPECTION_IMAGES.id를 참조한다. |
| ANALYSIS_JOBS | input_type | RGB_SINGLE / THERMAL_SINGLE |
| ANALYSIS_JOBS | requested_model_type | 사용자가 요청한 모델 유형 또는 자동 모델 라우팅 기준 |
| ANALYSIS_JOBS | model_type | 실제 실행된 모델 유형. RGB_ONLY / THERMAL_ONLY |
...
| 항목 | 설명 |
| --- | --- |
| ANALYSIS_JOBS의 대상 | ANALYSIS_JOBS는 inspection 전체가 아니라 image_id로 식별되는 이미지 한 건을 대상으로 생성한다. |
| ANALYSIS_JOBS 정규화 | ANALYSIS_JOBS에는 inspection_id를 직접 저장하지 않는다. 점검 회차는 image_id를 통해 조회한다. |
| 단건 분석 | RGB 또는 열화상 이미지 한 장을 image_id 기준으로 분석한다. |
| 모델 라우팅 | RGB 이미지는 RGB_ONLY 모델로, 열화상 이미지는 THERMAL_ONLY 모델로 처리한다. |
| 분석 재요청 | 동일 이미지에 대해 재요청할 경우 새로운 ANALYSIS_JOBS 레코드를 생성한다. |
...
| 결과 이미지 저장 | bbox, heatmap, mask 결과 이미지는 생성된 경우 객체 저장소에 저장하고, DB에는 bucket_name, object_key, file_url 형태의 경로만 저장한다. |
...
| Table | Columns | Description |
| --- | --- | --- |
| OPERATION_LOGS | id, actor_user_id, event_category, event_type, target_table, target_id, plant_id, zone_id, inspection_id, image_id, analysis_job_id, analysis_result_id, ip_address, user_agent, message, detail, created_at, updated_at | 로그인, 이미지 업로드, 분석 요청, 분석 실패, 결과 검토, 관리자 작업 등 주요 운영 이벤트를 기록한다. |
...
| 기록 대상 | 로그인, 이미지 업로드, 분석 요청, 분석 실패, 검토 상태 변경, 관리자 승인·비활성화 작업 등 |
...
| INSPECTION_IMAGES 1:N ANALYSIS_JOBS | RGB 또는 열화상 이미지 한 건을 기준으로 분석 작업을 생성한다. |
| ANALYSIS_JOBS 1:1 ANALYSIS_RESULTS | 분석 작업 하나는 결과 요약 하나를 가진다. 대상 정보는 ANALYSIS_JOBS의 image_id를 통해 조회한다. |
...
| OPERATION_LOGS optional FK | 운영 로그는 USERS, PLANTS, ZONES, INSPECTIONS, INSPECTION_IMAGES, ANALYSIS_JOBS, ANALYSIS_RESULTS를 선택적으로 참조한다. |
...
| 중복 FK 제거 | INSPECTION_IMAGES와 ANALYSIS_RESULTS에는 plant_id, zone_id 같은 조회용 중복 FK를 저장하지 않는다. |
| ZONE | 분석 결과를 직접 가지지 않고, INSPECTION이 생성되는 상위 관리 단위이다. |
| INSPECTIONS | 단순 이미지 묶음이 아니라 특정 구역에 대한 점검 회차 또는 점검 이벤트이다. INSPECTIONS는 zone_id만 직접 가진다. plant_id는 ZONES를 통해 조회한다. |
...
| ANALYSIS_JOBS | inspection 전체가 아니라 image_id를 대상으로 생성한다. inspection_id는 직접 저장하지 않는다. |
| 입력 유형 | input_type은 RGB_SINGLE 또는 THERMAL_SINGLE로 관리한다. |
| 모델 유형 | model_type은 RGB_ONLY 또는 THERMAL_ONLY로 관리한다. |
...
| Pair/Fusion | Pair 생성·관리와 Fusion 분석은 현재 운영 범위와 ERD에서 제외한다. |
...
| 제외 테이블 | 제외 이유 |
| --- | --- |
| IMAGE_PAIRS | Pair 생성·관리 및 Fusion 분석이 현재 운영 범위에서 제외되어 현재 스키마에서 사용하지 않는다. 과거 생성·제거 이력은 마이그레이션 기록으로 유지한다. |
...
OPERATION_LOGS
→ USERS / PLANTS / ZONES / INSPECTIONS / INSPECTION_IMAGES / ANALYSIS_JOBS / ANALYSIS_RESULTS
```