...
| `inspectionId` | 이미지 또는 Pair를 통해 조회되는 값                                |
| `targetType`   | 이미지 또는 Pair의 검사 대상 단위                                 |
| `equipmentId`  | Array, Panel, Module 대상일 때 사용, Zone 전체 대상이면 `null` 가능 |
| `imageId`      | `inspection_images.id`                                |
| `imagePairId`  | `image_pairs.id`                                      |
...
| Pair   | `IMAGE_PAIRS`는 `inspection_id`, `equipment_id`, `target_type`, `rgb_image_id`, `thermal_image_id`를 가진다. |
| 분석 작업  | `ANALYSIS_JOBS`는 `image_id` 또는 `image_pair_id` 중 하나를 대상으로 생성한다. `inspection_id`는 저장하지 않는다.              |
...
| 주요 역할 | 인증, 권한 검증, 발전소·구역·점검 관리, 이미지 메타데이터 관리, Pair 관리, 분석 작업 생성, 결과 조회 |
| 외부 노출 | 가능                                                              |
| 인증 필요 | 대부분 필요                                                          |
...
| 처리 방식 | SQS 기반 비동기 처리                                                |
| 주요 역할 | 분석 작업 메시지 전달, 객체 저장소 이미지 읽기, ONNX 추론, 결과 이미지 저장, 결과 메타데이터 저장 |
| 외부 노출 | 금지                                                           |
...
| API-PAIR-001        | Pair 후보 조회            |              GET | `/image-pairs/candidates`               |    - |     - |      O |   O |            - | 접근 가능 점검            |
| API-PAIR-002        | RGB-Thermal Pair 생성   |             POST | `/image-pairs`                          |    - |     - |      O |   O |            - | 동일 점검·동일 검사 대상      |
| API-PAIR-003        | Pair 상세 조회            |              GET | `/image-pairs/{imagePairId}`            |    - |     - |    조건부 |   O |            - | 접근 가능 Pair          |
| API-PAIR-004        | Pair 수정               |            PATCH | `/image-pairs/{imagePairId}`            |    - |     - |    조건부 |   O |            - | 접근 가능 Pair          |
| API-PAIR-005        | Pair 비활성화             |            PATCH | `/image-pairs/{imagePairId}/deactivate` |    - |     - |    조건부 |   O |            - | 접근 가능 Pair          |
| API-AI-JOB-001      | AI 분석 요청              |             POST | `/analysis-jobs`                        |    - |     - |      O |   O |            - | 접근 가능 이미지 또는 Pair   |
...
| 설명       | 점검 정보, 연결 이미지, Pair 정보, 분석 상태, 결과 요약을 조회한다. |
| 권한       | 일반 사용자, 관리자                                 |
| 성공 코드    | 200                                         |
| 관련 요구사항  | FR-025, FR-026                              |
...
| `images`           | 연결 이미지 목록               |
| `imagePairs`       | 연결된 RGB-Thermal Pair 목록 |
| `analysisJobs`     | 분석 작업 요약                |
...
## 8.8 RGB-Thermal Pair API

### API-PAIR-001. Pair 후보 조회

| 항목       | 내용                                                     |
| -------- | ------------------------------------------------------ |
| Method   | GET                                                    |
| Endpoint | `/api/v1/image-pairs/candidates`                       |
| 설명       | 동일 점검, 동일 검사 대상 기준으로 Pair 구성 가능한 RGB/열화상 이미지 후보를 조회한다. |
| 권한       | 일반 사용자, 관리자                                            |
| 성공 코드    | 200                                                    |
| 관련 요구사항  | FR-029                                                 |
...
| `inspectionId` | number |  Y | 점검 ID                         |
| `targetType`   | string |  Y | ZONE / ARRAY / PANEL / MODULE |
| `equipmentId`  | number |  N | 설비 ID. Zone 전체 대상이면 `null`    |
...
### API-PAIR-002. RGB-Thermal Pair 생성

| 항목       | 내용                                |
| -------- | --------------------------------- |
| Method   | POST                              |
| Endpoint | `/api/v1/image-pairs`             |
| 설명       | RGB 이미지와 열화상 이미지를 하나의 Pair로 연결한다. |
| 권한       | 일반 사용자, 관리자                       |
| 성공 코드    | 201                               |
| 관련 요구사항  | FR-029, FR-046                    |
...
| 항목       | 검증 내용                                               |
| -------- | --------------------------------------------------- |
| 이미지 유형   | `rgbImageId`는 RGB, `thermalImageId`는 THERMAL이어야 한다. |
| 점검       | 두 이미지의 `inspection_id`가 같아야 한다.                     |
| 검사 대상 단위 | 두 이미지의 `target_type`이 같아야 한다.                       |
| 검사 대상 위치 | 두 이미지의 `equipment_id`가 같거나 둘 다 `null`이어야 한다.        |
| 상태       | 두 이미지 모두 ACTIVE 상태여야 한다.                            |
| 중복 Pair  | 이미 다른 Pair에 연결된 이미지는 중복 연결하지 않는다.                   |
...
```json
{
  ...
  "message": "RGB-Thermal Pair가 생성되었습니다."
}
```

주의:

* `plantId`, `zoneId`는 저장 컬럼이 아니라 조회 편의용 파생 필드이다.
* Pair의 저장 기준은 `inspection_id`, `target_type`, `equipment_id`, `rgb_image_id`, `thermal_image_id`이다.

---

### API-PAIR-003. Pair 상세 조회

| 항목       | 내용                                  |
| -------- | ----------------------------------- |
| Method   | GET                                 |
| Endpoint | `/api/v1/image-pairs/{imagePairId}` |
| 설명       | Pair 정보와 연결된 RGB/열화상 이미지 정보를 조회한다.  |
| 권한       | 일반 사용자, 관리자                         |
| 성공 코드    | 200                                 |
| 관련 요구사항  | FR-029                              |
...
### API-PAIR-004. Pair 수정

| 항목       | 내용                                  |
| -------- | ----------------------------------- |
| Method   | PATCH                               |
| Endpoint | `/api/v1/image-pairs/{imagePairId}` |
| 설명       | Pair에 연결된 RGB 또는 열화상 이미지를 수정한다.     |
| 권한       | 일반 사용자, 관리자                         |
| 성공 코드    | 200                                 |
| 관련 요구사항  | FR-029                              |
...
### API-PAIR-005. Pair 비활성화

| 항목       | 내용                                             |
| -------- | ---------------------------------------------- |
| Method   | PATCH                                          |
| Endpoint | `/api/v1/image-pairs/{imagePairId}/deactivate` |
| 설명       | Pair 연결을 실제 삭제하지 않고 비활성화한다. 원본 이미지는 삭제하지 않는다.  |
| 권한       | 일반 사용자, 관리자                                    |
| 성공 코드    | 200                                            |
| 관련 요구사항  | FR-029                                         |
...
* 기존 분석 작업 또는 분석 결과가 연결된 Pair는 실제 삭제하지 않는다.
* Pair를 비활성화해도 RGB 이미지와 열화상 원본 이미지는 유지한다.
* Pair의 상태만 `INACTIVE`로 변경한다.
...
| 설명       | RGB 이미지, 열화상 이미지 또는 RGB-Thermal Pair에 대한 분석 작업을 생성한다. 실제 분석은 비동기로 처리한다. |
| 권한       | 일반 사용자, 관리자                                                             |
| 성공 코드    | 202                                                                     |
...
| 조건         | 기준                                  |
| ---------- | ----------------------------------- |
| 단건 분석      | `imageId` 필수, `imagePairId`는 `null` |
| Pair 분석    | `imagePairId` 필수, `imageId`는 `null` |
| RGB 단건     | 대상 이미지의 `imageType`이 RGB            |
| Thermal 단건 | 대상 이미지의 `imageType`이 THERMAL        |
| Fusion 분석  | 대상 Pair가 ACTIVE 상태                  |
| 중복 분석      | 동일 대상에 진행 중인 Job이 있으면 중복 생성 제한 가능   |
...
* 점검, 구역, 발전소 정보는 `imageId` 또는 `imagePairId`를 통해 조회한다.
...
| `inputType`    | string |  N | RGB_SINGLE / THERMAL_SINGLE / RGB_THERMAL_PAIR |
| `modelType`    | string |  N | RGB_ONLY / THERMAL_ONLY / FUSION               |
| `page`         | number |  N | 페이지 번호                                         |
| `size`         | number |  N | 페이지 크기                                         |
...
| 설명       | 실패했거나 재검토가 필요한 분석 작업을 다시 요청한다.        |
| 권한       | 일반 사용자, 관리자                           |
| 성공 코드    | 202                                   |
...
| 설명       | 접근 가능한 분석 결과 목록을 조회하고 조건별로 필터링한다. |
| 권한       | 일반 사용자, 관리자                       |
| 성공 코드    | 200                               |
...
* 위 필터들은 `analysis_job_id → analysis_jobs → inspection_images/image_pairs → inspections → zones → plants` 관계를 통해 조회한다.
...
| 설명       | 분석 결과, 조치 후보, 심각도, 우선순위, 모델 정보, 시각화 정보를 상세 조회한다. Pair 분석 결과가 있으면 Fusion 결과를 우선 표시한다. |
| 권한       | 일반 사용자, 관리자                                                                          |
| 성공 코드    | 200                                                                                  |
| 관련 요구사항  | FR-069                                                                               |
...
| `visualization`   | bbox, heatmap, mask 결과 이미지 정보                  |
...
```json
{
  ...
  "data": {
    ...
    "visualization": {
      ...
      "heatmapUrl": "/api/v1/results/5000/visualization?type=HEATMAP",
      "maskUrl": "/api/v1/results/5000/visualization?type=MASK"
    },
    "modelInfo": {
...
| 설명       | 권한 검증 후 분석 결과 시각화 이미지 URL 또는 스트림을 제공한다.        |
| 권한       | 일반 사용자, 관리자                                    |
| 성공 코드    | 200                                            |
...
```json
{
  ...
  "data": {
    "type": "HEATMAP",
    "url": "/api/v1/results/5000/visualization?type=HEATMAP&mode=STREAM",
    "expiresAt": "2026-06-01T10:10:00+09:00"
  },
...
| 설명       | 관리자가 전체 업로드 이미지 목록과 Pair 연결 정보를 조회한다. |
| 권한       | 관리자                                   |
| 성공 코드    | 200                                   |
| 관련 요구사항  | FR-035                                |
...
| 설명       | 로그인, 이미지 업로드, Pair 생성, 분석 요청, 분석 실패, 결과 검토, 관리자 작업 등 주요 운영 이벤트 로그를 조회한다. |
| 권한       | 관리자                                                                      |
| 성공 코드    | 200                                                                      |
| 관련 요구사항  | NFR-063                                                                  |
...
Backend는 분석 작업 생성 후 SQS에 다음 메시지를 등록한다.
...
AI Worker가 분석 성공 시 저장 또는 전달해야 하는 결과 구조이다.
...
```json
{
  ...
  "result": {
    ...
    "visualization": {
      ...
      "heatmapObjectKey": "results/5000/heatmap.jpg",
      "heatmapFileUrl": "/api/v1/results/5000/visualization?type=HEATMAP",
      "maskBucketName": "pv-results",
      "maskObjectKey": "results/5000/mask.png",
      "maskFileUrl": "/api/v1/results/5000/visualization?type=MASK"
    },
    "detections": [
      {
        "defectType": "HOTSPOT",
...
| `RGB_THERMAL_PAIR` | RGB-Thermal Pair 입력 |
...
| `FUSION`       | RGB-Thermal Fusion 분석 모델 |

## 10.9 RequestedModelType

| 값              | 설명                 |
| -------------- | ------------------ |
| `AUTO`         | 입력 유형에 따라 자동 선택    |
| `RGB_ONLY`     | RGB-only 모델 요청     |
| `THERMAL_ONLY` | Thermal-only 모델 요청 |
| `FUSION_AUTO`  | Fusion 모델 자동 선택    |
| `EARLY_FUSION` | Early Fusion 요청    |
| `LATE_FUSION`  | Late Fusion 요청     |
...
|         400 | `PAIR_CONDITION_MISMATCH`      | Pair 생성 조건 불일치           |
|         400 | `INVALID_ANALYSIS_TARGET`      | 분석 대상이 유효하지 않음           |
...
|         409 | `PAIR_ALREADY_EXISTS`          | 이미 Pair가 존재함             |
|         409 | `ANALYSIS_JOB_ALREADY_RUNNING` | 동일 대상 분석 작업 진행 중         |
...
| Pair 생성         | 동일 발전소·구역·점검·검사 대상 기준으로 넓게 표현                             | 두 이미지의 `inspection_id`, `target_type`, `equipment_id` 일치 검증     |
| Pair 해제         | `DELETE /image-pairs/{imagePairId}`                       | `PATCH /image-pairs/{imagePairId}/deactivate`                   |
| 분석 Job          | `inspectionId` 필터와 저장 기준 혼동                               | `imageId` 또는 `imagePairId` 대상 생성, `inspectionId`는 Join 필터       |
| 분석 Job 상태       | `status` 혼용                                               | `jobStatus`로 통일                                                 |
| 분석 Result       | `plantId`, `zoneId`, `inspectionId`, `targetType` 저장처럼 표현 | `analysisJobId` 기준 저장, 대상 정보는 파생 조회                             |
| 이미지 조회          | JSON 응답과 Stream 응답 혼재 가능                                  | URL 응답과 Stream 응답 기준 분리                                         |
| 상태값             | `WAITING`, `PAIR`, `RGB`, `THERMAL` 혼재 가능                 | `QUEUED`, `RGB_SINGLE`, `THERMAL_SINGLE`, `RGB_THERMAL_PAIR` 기준 |
...
| 운영 로그           | 별도 관리자 로그 가능성                                             | `OPERATION_LOGS`로 통합                                            |
