...
| AI Experiment | Ultralytics는 `8.2.x`처럼 열어두지 않고 세부 버전까지 고정 |
...
| Queue | AWS SQS | 관리형 서비스 | 운영 비동기 분석 큐 |
...
| Python Library | FastAPI | 0.115.5 | AI Worker API / 내부 처리 |
...
| Python Library | python-multipart | 0.0.12 | 파일 업로드 처리 |
...
| Backend Callback | AI Worker가 Backend Internal API로 결과를 전달하는 구조라면 DB 접근 의존성은 최소화 가능하다 |
...
| Model Format | ONNX FP32 | 모델별 관리 | 기본 배포 후보 |
...
| Python Library | ultralytics | 8.2.10 | YOLOv8n / YOLOv8s 실험 기준 |
| Python Library | anomalib | requirements-anomaly.txt 기준 | Torch / Lightning 조합 검증 후 별도 고정 |
...
| Ultralytics | `ultralytics 8.2.x`처럼 넓게 열지 않고 `ultralytics==8.2.10`으로 고정한다 |
| Anomalib | Torch, Lightning 의존성이 강하므로 YOLO 실험 환경과 분리한다 |
...
| 환경 | 목적 | 파일 |
| --- | --- | --- |
| YOLO 실험 환경 | RGB-only / Thermal-only / Fusion 탐지 실험 | `experiments/requirements-train.txt` |
...
# Anomalib은 YOLO 학습 환경과 분리된 별도 venv에서 설치한다.
...
| AI Experiment | `experiments/requirements-train.txt` | YOLO / Fusion 학습 의존성 |
| AI Experiment | `experiments/requirements-anomaly.txt` | Anomalib 품질 검증 의존성 |
...
| Model | `model-manifest.yaml` | 모델명, 버전, Runtime, threshold 관리 |
...
또는 사용자 환경에서 다르게 출력되는 경우 해당 출력값을 그대로 기록한다.
...
| Queue | AWS SQS | 관리형 서비스 | AI 분석 작업 큐 |
| Registry | AWS ECR | 관리형 서비스 | Docker 이미지 저장소 |
...
| 모델 파일 | Docker 이미지에 포함하지 않고 AI Worker 기동 시 S3에서 다운로드한다 |
...
| AI Experiment | `experiments/requirements-train.txt` | YOLO / Fusion 학습 의존성 |
| AI Experiment | `experiments/requirements-anomaly.txt` | Anomalib 전용 의존성 |
| AI Experiment | `experiments/requirements-anomaly.lock.txt` | Anomalib 실제 설치 버전 고정 |
| AI Experiment | `experiments/requirements.lock.txt` | 실험 재현성 확보 |
| Model | `model-manifest.yaml` | 모델명, 버전, Runtime, threshold 관리 |
...
| AI Experiment | ultralytics 8.2.x 범위 설치 | `ultralytics==8.2.10`으로 세부 버전 고정 |
| AI Experiment | torch / anomalib 동시 x | Anomalib 환경 분리 및 lock 파일 생성 |
...
models:
  - model_name: rgb-only-yolov8s
    ...
    input_size: 640
    threshold: 0.25
    object_key: models/rgb-only-yolov8s/v0.1.0/model.onnx

  - model_name: thermal-only-yolov8s
    model_version: v0.1.0
    ...
    input_size: 640
    threshold: 0.25
    object_key: models/thermal-only-yolov8s/v0.1.0/model.onnx

  - model_name: rgb-thermal-fusion-yolov8s
    model_version: v0.1.0
    input_type: RGB_THERMAL_PAIR
    model_type: FUSION
    format: ONNX
    precision: FP32
    runtime: ONNX_RUNTIME
    input_size: 640
    threshold: 0.25
    object_key: models/fusion-yolov8s/v0.1.0/model.onnx
```
...
| 9 | AI Experiment requirements-train.txt | `ultralytics==8.2.10`으로 세부 버전 고정 |
| 10 | AI Experiment requirements 분리 | YOLO / Anomalib / Export 충돌 방지 |
...
- 기술 스택 및 Dependency 버전 관리 문서 최종본을 정리함.
- Frontend는 TypeScript 5.6.3, ESLint 8.57.1, @typescript-eslint 8.10.0 이상 조합을 유지함.
- Backend는 Spring Boot 3.5.14를 사용하고 실제 build.gradle에도 같은 버전으로 고정하도록 명시함.
- Gradle `io.spring.dependency-management` 플러그인은 `1.1.5`로 정확히 표기함.
- Flyway는 PostgreSQL 16 대응을 위해 `flyway-core`와 `flyway-database-postgresql`을 함께 사용하고 같은 버전으로 관리하도록 유지함.
- JJWT는 0.12.6으로 고정함.
- AI Server와 AI Experiment는 NumPy 1.26.4 기준으로 통일함.
- AI Experiment의 Ultralytics는 `8.2.x`가 아니라 `8.2.10`으로 세부 버전 고정함.
- Anomalib은 YOLO 학습 환경과 분리하고, `requirements-anomaly.txt`에서 `==x` 표기를 제거함.
- LocalStack은 `localstack/localstack:3.5.0`, MinIO는 `minio/minio:RELEASE.2024-07-16T23-46-41Z`, PostgreSQL Docker는 `postgres:16`으로 고정함.
- Docker Compose 버전은 `docker compose version` 출력값 기준으로 재확인하도록 정리함.