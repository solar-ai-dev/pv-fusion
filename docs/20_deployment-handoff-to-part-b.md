...
> [문서 상태]
>
> 이 문서는 B 담당의 실제 AWS 배포 작업을 시작하기 전에 작성한 인계용 문서이다.
>
> 해당 인계와 초기 배포 작업은 완료되었으며, 현재 운영 기준을 정의하는 최신 문서로 사용하지 않는다.
>
> 아래의 역할 분담, 미수행 범위, 배포 순서, 확인 필요 항목은 **문서 작성 당시의 상태와 작업 계획을 기록한 내용**이다. 현재 배포 상태를 판단하는 기준으로 사용하지 않는다.
>
> 현재 배포·환경변수·Docker 이미지·Jenkins CI/CD 기준은 다음 문서를 우선한다.
>
> - `docs/12_cloud-deployment-operations-design.md`
> - `docs/17_environment-variable-secret-contract.md`
> - `docs/18_service-docker-image-contract.md`
> - `docs/19_jenkins-ecr-ci-preparation.md`
>
> 실제 현재 상태는 AWS 리소스, ECR 이미지, K3s 매니페스트, Jenkins 설정과 최신 배포 결과 기록을 함께 확인한다.
>
> 이 문서는 배포 준비와 인계 과정의 작업 이력을 확인하기 위한 보관용 자료로 유지한다.
