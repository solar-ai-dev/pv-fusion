...
class PlantPersistenceAdapterTest extends PostgresTestContainerSupport {
    ...
    void returnsZoneCountAndLatestInspectionAtInSummary() {
        ...
        Long zoneId = insertZone(plant.getId(), ownerUser.getId(), "Zone A");
        insertZone(plant.getId(), ownerUser.getId(), "Zone B");
        insertInspection(zoneId, ownerUser.getId(), "2026-06-05T12:00:00+09:00");
    ...
    private Long insertZone(Long plantId, Long userId, String name) {
        Number zoneId = (Number) entityManager.createNativeQuery("""
